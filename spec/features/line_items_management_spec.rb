...
RSpec.describe 'line items management', type: :feature do
  ...
  it 'creates line item' do
    ...
    expect(page).to have_selector("#line_item_date_#{line_item_date.id}_line_items .line-item", count: 1)
  end
  ...
  context 'when deleting a date' do
    ...
    it 'deletes quote' do
      ...
      expect(page).to have_selector("#line_item_date_#{line_item_date.id}_line_items .line-item", count: 0)
    end
  end
end
